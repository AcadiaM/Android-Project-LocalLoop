...
import com.example.local_loop.Account.Session;
...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Session startSesion(int userID){
        SQLiteDatabase db = getReadableDatabase();
        Session session = null;

        Cursor cursor = db.query("users", new String[]{"role", "username","firstName","lastname"}, "id = ?", new String[]{String.valueOf(userID)}, null, null, null);
        ...
        if (cursor.moveToFirst()) {
            ...
            session = new Session(userID, role, username,firstName,lastName);
        }
        if (cursor != null) {
            cursor.close();
        }
    ...
    public Session checkLogin(String username, String password) {
        SQLiteDatabase db = this.getReadableDatabase();
        Session session = null;

        Cursor cursor = db.query(TABLE_USERS, new String[]{"id", "role", "username", "firstName", "lastname"}, "username = ? AND password = ? AND active = ?", new String[]{username, password, "1"}, null, null, null);
        ...
        if (cursor.moveToFirst()) {
            ...
            session = new Session(id, role, dbUsername, firstName, lastName);
        }
